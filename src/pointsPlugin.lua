...
function script.drawUI()
    if uiVisible then
        ...
        local function speedMeter(ref)
            ui.drawRectFilled((ref) + vec2(0 + xLvl, -4), ref + vec2(180, 5), colorDark, 1)
            ui.drawLine(ref + vec2(0 + xLvl, -4), ref + vec2(0, 4), colorGrey, 1)
            ui.drawLine(ref + vec2(requiredSpeed + xLvl, -4), ref + vec2(requiredSpeed, 4), colorGrey, 1)

            local speed = math.min(ac.getCarState(1).speedKmh, 180)
            if speed > 1 then
                ui.drawLine(ref + vec2(0 + xLvl, 0), ref + vec2(speed, 0), colorAccent, 4)
            end
        end

        ui.beginTransparentWindow("overtakeScore", vec2(100 + xLvl, 100), vec2(400 * 0.5, 400 * 0.5))
        ui.beginOutline()
