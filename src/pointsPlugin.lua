...
local filePath = "D:\Assetto Corsa\Our Plugins\high_scores.txt"
local highScores = {}
local UserDataFilePath: "user_data.txt"
...
function storeUserData(discordId, steamId, playerName, score)
local userData = {
discordId = discordId,
steamId = steamId,
...
-- Load high scores on script initialization
loadHighScores()
end
...
local comboMeter = 1
local comboColor = 0
local highestScore = loadHighScores
...
local player = ac.getCarState(1)
if player.engineLifeLeft < 1 then
if totalScore > (highScores[player.steamId] or 0) then
highScores[player.steamId] = math.floor(totalScore)
ac.sendChatMessage("scored " .. totalScore .. " points.")
saveHighScores()  -- Save high scores after updating
end
totalScore = 0
comboMeter = 1
return
...
ui.textAligned("Highest Score: " .. highScores[steamId] .. " pts", vec2(100, 100))
ui.popFont()
ui.popStyleVar()
